...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $image_name = uniqid(mt_rand(), true) . '.' . substr(strrchr($_FILES['image']['name'], '.'), 1); //マイクロ秒単位の現在時刻にもとづいた、接頭辞つきの一意な ID を取得し、ユニークな値を生成できる
  //substr以降でアップロードされたファイルの拡張子を取得
  ...
  $lastInsertId = $pdo->lastInsertId(); //最後に登録したデータのIDを取得する
  ...
  for($i = 0; $i < count($_POST["choices"]); $i++){
    $stmt->execute([
      ...
      "valid" => (int)$_POST['correctChoice'] === $i + 1 ? 1 : 0, //choiceの配列のうち、index何番めが正解か、もし正解と同じ番号の問題であったらvalidに1を入れる
      "question_id" => $lastInsertId
    ]);
  }
  header("Location: " . "/learning_admin/questions.php");
  ...
  <script>
    ...
    inputDoms.forEach(inputDom => {
      inputDom.addEventListener('input', event => {
        const isFilled = inputDoms.filter(d => d.value).length === inputDoms.length //クラス名required（formの内容）が存在する部分全てが答えられているかを判断する
        submitButton.disabled = !isFilled //中身がある場合はdisabledを解除
      })
    })
  </script>
</body>
