...
$_POST['id'] = parse_url($url, PHP_URL_QUERY);


if (!isset($_SESSION['id'])) {
  header('Location: /learning_admin/auth/signin.php');
} else {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_FILES["image"]["tmp_name"] !== "") {
      $set_query .= ", image = :image"; //画像が追加されている場合はupdate文にimageも登録するように変更
      $params["image"] = "";
    }
  ...
  <div class="wrapper">
    ...
    <main>
      <div class="container">
        ...
        <!-- <form action="#" class="question-form" method="POST" enctype="multipart/form-data"> -->
        <form action="../../services/update_question.php" class="question-form" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-4">
            ...
            <?php foreach ($choices as $key => $choice) { ?>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="correctChoice" id="correctChoice<?= $key ?>" value="<?= $key + 1 ?>" <?= $choice["valid"] === 1 ? 'checked' : '' ?>>
          ...
          <div class="mb-4">
            ...
            <input type="file" name="image" id="image" class="form-control" placeholder="問題文を入力してください" />
          </div>
          <div class="mb-4">
