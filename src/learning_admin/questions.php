...
if(!isset($_SESSION['id'])) {
  ...
  if (isset($_SESSION['message'])) {
    ...
    unset($_SESSION['message']); //unsetは指定した変数を破棄する
  }
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      $stmt->bindValue(":question_id", $_SESSION["id"]);
      $stmt->execute();
...
<head>
  ...
  <script src="../assets/scripts/sample.js" defer></script>
</head>
<body>
              ...
              <tbody>
                <?php foreach($questions as $question) { ?>
                  <tr id="question-<?= $question["id"] ?>">
                    <td><?= $question["id"]?></td>
                    <td>
                      <a href="/learning_admin/questions/edit.php?<?= $question["id"] ?>">
                      <?= $question["content"] ?>
                    </a>
                    </td>
                    <td>
                      <form action="./questions/edit.php" method="post">
